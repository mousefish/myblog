---
title: 'WebGPU 学习笔记 1 - Hello Triangle'
date: '2022-08-20'
tags: ['webgpu']
draft: false
...
- `navigator.gpu` is the entry point of the webgpu, if navigator.gpu not in navigator, normally just because webgpu is not supported in the current webbrowser. currently when I write this article,
  the webgpu still under development, so you need to either download a Chrom Canary verion or apply for a trail
...
- vertex buffer pass the 3 points position of triangle and the color to fragment, and fragment output color to draw pixels.
