...
我们只需要在pages的文件夹下简历news.tsx文件，Next.js会自动将在pages文件夹内的news.tsx文件转换成相应页面路由xxx.com/new, 我们无需额外设置路由。

- 如果我们有个动态网页页面: xxx.com/news/id
我们只需要在pages文件夹内新建news文件夹，然后建立一个[id].tsx的文件，next会基于不同的id来建立相应页面的路由。比如如果用户点击id=1的news内容，路由生成将为xxx.com/news/1. 使用路径：pages/news/[id]/index.tsx 也可以达到相同目的。 

以下代码为如何从路由中获取动态的id数值
```javascript
...
Next.js其中非常重要的一个功能为页面静态生成和服务器端预渲染。 这个技术能让网页在服务器生成后才传达到客户端显示， 它能为搜索引擎提供更多的内容搜索信息， 并且让客户端更快速的加载页面。 
...
我们用 xxx.com/news/id 作为例子, 如果需要生成所有id相应的页面，我们要在news/[id].tsx文件中加入以下代码：
